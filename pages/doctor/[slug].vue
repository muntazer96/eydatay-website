...
if (!doctorId.value) {
  throw createError({
    statusCode: 404,
    statusMessage: "الطبيب غير موجود",
    fatal: true,
  });
}
...
const {
  ...
  pending,
  error,
} = await useAsyncData<PublicDoctorProfileDto>(`doctor-${doctorId.value}`, () =>
...
const governorate = computed(
  () => primaryClinic.value?.iraqiProvinceName ?? null,
);
...
const doctorSlugPath = computed(() =>
  doctorSlug(doctor.value!.id, doctor.value!.normalizedName),
);
...
const pageTitle = computed(() =>
  doctor.value ? doctorPageTitle(doctor.value) : "طبيب",
);
...
useHead({
  ...
  meta: [
    {
      name: "description",
      content: doctor.value ? doctorPageDescription(doctor.value) : "",
    },
    ...
    {
      property: "og:image",
      content: doctorCardImage(doctor.value) ?? siteAssetUrl("/onWhiteBG.png"),
    },
    { property: "profile:first_name", content: doctor.value?.name ?? "" },
  ],
  link: [
    { rel: "canonical", href: siteUrl(`/doctor/${doctorSlugPath.value}`) },
  ],
  script: [
    {
  ...
  const clinics = (d.clinics ?? []).map((c) => {
    ...
    if (c.address)
      node.address = {
        ...
        addressRegion: c.iraqiProvinceName || undefined,
      };
    return node;
  });
...
function handleExternalLink(link: DoctorExternalLinkDto) {
  if (doctor.value)
    useAnalytics().track("external_link_click", { doctorId: doctor.value.id });
}
...
function groupSchedule(availabilities: PublicClinicAvailabilityDto[]) {
  ...
  return days.map((day) => {
    ...
    return {
      day,
      rows: rows.map((r) => ({
        start: formatTimeSpan(r.startTime),
        end: formatTimeSpan(r.endTime),
      })),
    };
  });
}
      ...
      <div class="profile-grid">
        ...
        <div class="profile-main">
          ...
          <section class="card profile-head" aria-labelledby="doctor-name">
              ...
              <div class="profile-identity">
                  ...
                  <BaseIcon
                    :name="
                      specializationIconName(
                        doctor.specializationNormalizedName,
                      )
                    "
                    :size="18"
                  />
                ...
                <div class="profile-stats">
                  <RatingStars
                    ...
                    :review-count="doctor.reviewCount"
                  />
            ...
            <p v-if="doctor.description" class="profile-desc">
              {{ doctor.description }}
            </p>
          ...
          <section
            ...
            class="card social-block"
            aria-labelledby="doc-social"
          >
            ...
            <div class="social-list">
              ...
              >
                <BaseIcon
                  :name="linkTypeIcon[link.type]?.icon ?? 'link-variant'"
                  :size="22"
                />
                <span>{{
                  link.displayName || linkTypeIcon[link.type]?.label || "رابط"
                }}</span>
                <BaseIcon
                  class="social-list__arrow"
                ...
                <div class="clinic__main">
                  ...
                  <ul class="clinic__meta">
                    <li>
                      ...
                      <span>{{
                        clinic.address || "لم يُحدد العنوان بعد"
                      }}</span>
                    </li>
                  ...
                  <div class="clinic__schedule">
                    <template
                      v-for="{ day, rows } in groupSchedule(
                    ...
                    >
                      <div v-if="rows.length" class="schedule-row">
                        ...
                        <span class="schedule-row__times">
                          ...
                          <span v-if="!rows.length" class="schedule-row__none"
                            >غير متاح</span
                          >
                        </span>
                      </div>
                    ...
                    <p
                      v-if="!(clinic.availabilities ?? []).length"
                      class="clinic__none"
                    >
                  ...
                  <div class="clinic__actions">
                    ...
                    <span v-else-if="clinic.mapUrl" class="clinic__map-hint"
                      >موقع الخريطة متاح في تطبيق عيادتي</span
                    >
                  </div>
                </div>
              </article>
                ...
                <div class="review__top">
                  ...
                  <div class="review__author">
                    <span class="review__name">{{
                      review.user?.name || "مريض عبر تطبيق عيادتي"
                    }}</span>
                    <RatingStars :rating="review.rating" :size="16" />
                  </div>
                </div>
                <p v-if="review.comment" class="review__body">
                  {{ review.comment }}
                </p>
              </article>
            </div>
            <p v-else class="reviews-none">
              التقييمات والمراجعات تظهر بعد التحقق منه في تطبيق عيادتي — جرّب
              التطبيق لمشاركة تجربتك.
            </p>
          </section>
        </div>
        ...
        <aside class="profile-side">
          <section class="card book-card">
            ...
            <p v-if="doctor.canBookOnline" class="book-card__text">
              هذا الطبيب يتيح الحجز الإلكتروني عبر تطبيق عيادتي. اختر اليوم
              والوقت المناسبين.
            </p>
            <p v-else class="book-card__text">
              الحجز متاح حالياً عبر الهاتف أو زيارة العيادة. تظهر معلومات الحجز
              في التطبيق.
            </p>
            ...
            <template v-if="doctor.canBookOnline">
              <div class="book-card__doctor">
                ...
                />
                <span>{{
                  doctor.canBookOnline ? "احجز موعدك لتوفر المقعد" : ""
                }}</span>
              </div>
              <button
                type="button"
